...
import miniplc0java.analyser.o0.GlobalDef;
...
import static miniplc0java.analyser.IdentType.strToIdentType;
...
public final class Analyser {
    ...
    private OperandItem analysePrimaryExpr(String funcName, int funcNo) throws CompileError {
        /*
         * primary_expr literal_expr -> UINT_LITERAL | DOUBLE_LITERAL | STRING_LITERAL |
         * CHAR_LITERAL call_expr -> IDENT '(' call_param_list? ')' assign_expr -> IDENT
         * '=' expr ident_expr -> IDENT negate_expr -> '-' expr group_expr -> '(' expr
         * ')'
         */
        ...
        if (check(TokenType.UINT_LITERAL)) { // UINT_LITERAL
            ...
            throw new ExpectedTokenError(Arrays.asList(TokenType.UINT_LITERAL, TokenType.DOUBLE_LITERAL,
                    TokenType.STRING_LITERAL, TokenType.CHAR_LITERAL, TokenType.IDENT, TokenType.MINUS), peek());
        }
    }
    ...
    private OperandItem analyseExprOPG(String funcName, int funcNo) throws CompileError {
        ...
        /*
         * operator_expr -> expr binary_operator expr as_expr -> expr 'as' ty
         */
    ...
    private ArrayList<Integer> analyseBreakStmt(int funcNo, ArrayList<Integer> brBreaks) throws CompileError {
        expect(TokenType.BREAK);
        expect(TokenType.SEMICOLON);
        int brBreak = this.binCodeFile.addInstruction(funcNo, createInstruction(Operation.BR, 0));
        brBreaks.add(brBreak);
        return brBreaks;
    }
    ...
    private ArrayList<Integer> analyseWhileStmt(String funcName, int funcNo, ArrayList<Integer> brBreaks) throws CompileError {
        ...
        int endOffset = this.binCodeFile.addInstruction(funcNo, createInstruction(Operation.BR, 0));
        for (int brBreak : brBreaks) {
            this.binCodeFile.getInstruction(funcNo, brBreak).setParam(endOffset - brBreak - 1);
        }
